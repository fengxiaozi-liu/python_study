...
堆排序
    ...
    堆排序的过程
        ...
        5.重复步骤3，知道堆变空
    堆排序的核心
        1.建立一个向下调整的函数 ---> 这个向下调整的函数可以用来建立大根堆，在大根度建立完成之后还可以进行排序。
        2.堆顶元素和最后一个元素交换，在每一次交换完成之后都要更新，最后一个元素
        3.low代表堆顶元素，high代表堆底元素
    堆排序总结：
        1.堆排序需要向下调整的函数
        2.堆排序在排序的时候，为了更小的节省空间，不会将第一次取出来的最大元素放在一个新的列表中，而是存储在当前列表
          的最后一个元素，因此对于一个大根堆，就变成了正序排列，如果是一个小根堆就变成了倒叙
        3.堆排序总的来说是将数从大到小排列，列表的底部作为有序区，有序区的前面作为无序区
...
def sift(li, low, high):
    ...
    while j <= high:  # 只要j位置有数就一直循环
        if j + 1 <= high and li[j + 1] > li[j]:  # 如果右孩子节点比左孩子大,还有一个必要的前提是保证右孩子要有
            j = j + 1  # 将j指向右孩子
        if li[j] > temp:
            li[i] = li[j]
            # 交换完成之后往下看一层
            ...
            # 当不发生调整的时候，说明这是一个大根堆
            li[i] = temp  # 将temp放到最后相应的节点上
            break
    else:
        li[i] = temp  # j>high的时候，已经没有节点可以和i交换了，所以将temp节点放到指定的位置上就可以
...
def heap_sort(li):
    ...
    for i in range(n, -1, -1):
        # i永远指向堆的最后一个位置
        li[0], li[i] = li[i], li[0]
        sift(li, 0, i-1)  # 这时候i应该改变一下，变成一下新的最后一个元素
        print(li)
    return li


li = [3, 8, 7, 6, 5, 0, 1, 2, 4, 9]
print(heap_sort(li))
